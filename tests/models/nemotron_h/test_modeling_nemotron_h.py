...
from transformers import AutoTokenizer, NemotronHConfig, is_torch_available
...
class NemotronHConfigTester(ConfigTester):
...
class NemotronHModelTester:
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = NemotronHForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
...
class NemotronHModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {
            ...
            "text-classification": NemotronHForSequenceClassification,
            "text-generation": NemotronHForCausalLM,
            "zero-shot": NemotronHForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        ...
        self.config_tester = NemotronHConfigTester(self, config_class=NemotronHConfig, hidden_size=37)
    ...
    def test_mismatched_shapes_have_properly_initialized_weights(self):
        ...
        """
        self.skipTest(reason="Cumbersome and redundant for NemotronH")
    ...
    def test_flash_attn_2_fp32_ln(self):
        r"""
        Overriding the test_flash_attn_2_fp32_ln test as the NemotronH model, like Mixtral, doesn't support
    ...
    def test_flash_attn_2_inference_equivalence_right_padding(self):
        r"""
        Overriding the test_flash_attn_2_inference_padding_right test as the NemotronH model, like Mixtral, doesn't support
        right padding + use cache with FA2
        """
        self.skipTest(reason="NemotronH flash attention does not support right padding")
...
class NemotronHModelIntegrationTest(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        model_id = "ai21labs/NemotronH-tiny-dev"
        cls.model = NemotronHForCausalLM.from_pretrained(
            model_id,
