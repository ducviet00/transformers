...
# Adapted from transformers.models.mistral.modeling_mistral.MistralAttention with Mistral->NemotronH
...
# Adapted from transformers.models.mistral.modeling_mistral.MistralFlashAttention2 with Mistral->NemotronH
class NemotronHFlashAttention2(NemotronHAttention):
    """
    NemotronH flash attention module. This module inherits from `NemotronHAttention` as the weights of the module stays
...
# Adapted from transformers.models.mistral.modeling_mistral.MistralSdpaAttention with Mistral->NemotronH
class NemotronHSdpaAttention(NemotronHAttention):
    """
    NemotronH attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `NemotronHAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """
...
# Adapted from transformers.models.mixtral.modeling_mixtral.MixtralSparseMoeBlock with Mistral->NemotronH
class NemotronHSparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config: NemotronHConfig):
        ...
        self.experts = nn.ModuleList([NemotronHMLP(config) for _ in range(self.num_experts)])
...
class NemotronHAttentionDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: NemotronHConfig, layer_idx: int):
        super().__init__()
        num_experts = config.layers_num_experts[layer_idx]
        self.self_attn = NEMOTRON_H_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)

        ffn_layer_class = NemotronHSparseMoeBlock if num_experts > 1 else NemotronHMLP
        self.feed_forward = ffn_layer_class(config)
        self.input_layernorm = NemotronHRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.pre_ff_layernorm = NemotronHRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class NemotronHMambaMixer(nn.Module):
    """
    Compute ∆, A, B, C, and D the state space parameters and compute the `contextualized_states`.
    A, D are input independent (see Mamba paper [1] Section 3.5.2 "Interpretation of A" for why A isn't selective)
    ∆, B, C are input-dependent (this is a key difference between Mamba and the linear time invariant S4,
    ...
    def __init__(self, config: NemotronHConfig, layer_idx):
        ...
        self.dt_layernorm = NemotronHRMSNorm(self.time_step_rank, eps=config.rms_norm_eps)
        self.b_layernorm = NemotronHRMSNorm(self.ssm_state_size, eps=config.rms_norm_eps)
        self.c_layernorm = NemotronHRMSNorm(self.ssm_state_size, eps=config.rms_norm_eps)
...
class NemotronHMambaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: NemotronHConfig, layer_idx: int):
        super().__init__()
        num_experts = config.layers_num_experts[layer_idx]
        self.mamba = NemotronHMambaMixer(config=config, layer_idx=layer_idx)
...
class NemotronHPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["NemotronHAttentionDecoderLayer", "NemotronHMambaDecoderLayer"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, NemotronHRMSNorm):
            module.weight.data.fill_(1.0)
        elif isinstance(module, NemotronHMambaMixer):
...
ALL_DECODER_LAYER_TYPES = {"attention": NemotronHAttentionDecoderLayer, "mamba": NemotronHMambaDecoderLayer}


# Adapted from transformers.models.mistral.modeling_mistral.MistralModel with MISTRAL->NEMOTRON_H, Mistral->NemotronH
@auto_docstring
class NemotronHModel(NemotronHPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`NemotronHDecoderLayer`]
    ...
    def __init__(self, config: NemotronHConfig):
        ...
        self.final_layernorm = NemotronHRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache and past_key_values is None:
            logger.warning_once(
                "NemotronH requires an initialized `HybridMambaAttentionDynamicCache` to return a cache. None was "
        ...
        for decoder_layer in self.layers:
            ...
            layer_mask = mamba_mask if isinstance(decoder_layer, NemotronHMambaDecoderLayer) else causal_mask
...
# Adapted from transformers.models.mixtral.modeling_mixtral.MixtralForCausalLM with MIXTRAL->NEMOTRON_H, Mixtral->NemotronH
class NemotronHForCausalLM(NemotronHPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> model = NemotronHForCausalLM.from_pretrained("ai21labs/NemotronH-v0.1")
        >>> tokenizer = AutoTokenizer.from_pretrained("ai21labs/NemotronH-v0.1")
...
class NemotronHForSequenceClassification(GenericForSequenceClassification, NemotronHPreTrainedModel):
    pass


__all__ = ["NemotronHForCausalLM", "NemotronHForSequenceClassification", "NemotronHModel", "NemotronHPreTrainedModel"]
